...
public class UsersDbClient implements UsersClient {
    ...
    private final UserDataRepository userdataUserRepo = new UserDataRepositoryJdbc();
    ...
    public UserJson createUser(String username, String password) {
        return xaTransactionTemplate.execute(() -> {
                    ...
                    return UserJson.fromEntity(
                            userdataUserRepo.createUser(userEntity(username))
                    );
                }
        );
    }
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                xaTransactionTemplate.execute(() -> {
                            ...
                            authUserRepo.create(authUser);
                            UserEntity addressee = userdataUserRepo.createUser(userEntity(username));
                            userdataUserRepo.addFriendshipRequest(addressee, targetEntity);
                            return null;
                        }
                );
            }
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                xaTransactionTemplate.execute(() -> {
                            ...
                            authUserRepo.create(authUser);
                            UserEntity addressee = userdataUserRepo.createUser(userEntity(username));
                            userdataUserRepo.addFriendshipRequest(targetEntity, addressee);
                            return null;
                        }
                );
            }
